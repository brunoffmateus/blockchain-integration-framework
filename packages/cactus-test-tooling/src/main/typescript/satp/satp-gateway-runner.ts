...
export const SATP_GATEWAY_RUNNER_DEFAULT_OPTIONS = Object.freeze({
  ...
  envVars: [
    ...
    "SATP_GATEWAY_ADDRESS=https://localhost:3010",
    "SATP_PROOF_ID=proof456",
...
export class SATPGatewayRunner implements ITestLedger {
  ...
  constructor(
    ...
    this.grpcPort =
      options.grpcPort || SATP_GATEWAY_RUNNER_DEFAULT_OPTIONS.grpcPort;
    this.envVars =
  ...
  public async getGrpcHost(): Promise<string> {
    const containerInfo = await this.getContainerInfo();
    const hostPort = await Containers.getPublicPort(
      this.grpcPort,
      containerInfo,
