...
const logLevelInfo: LogLevelDesc = "INFO";
...
describe("Pausing all SATP bridges at once", () => {
  ...
  it("Should connect the gateway with Hephaestus", async () => {
    ...
    dispatcher!.setNonConformingTxObservable(nonConformedTxObservable);
  ...
  it("should accept a new correct transaction with Hephaestus", async () => {
    ...
    expect(responseBalanceOwner).toBeTruthy();
    expect(responseBalanceOwner.success).toBeTruthy();
    expect(responseBalanceOwner.callOutput).toBe("10");
    log.info("Amount was transfer correctly from the Owner account");
    ...
    expect(responseBalance2.status).toBeLessThan(300);
    expect(responseBalance2.data).not.toBeUndefined();
    expect(responseBalance2.data.functionOutput).toBe("4");
    log.info("Amount was transfer correctly to the Owner account");
  });

  it("should pause the bridges automatically due to non-conformance", async () => {
    hephaestus.newCaseId("tx5");
    // unmodeled behaviour:
