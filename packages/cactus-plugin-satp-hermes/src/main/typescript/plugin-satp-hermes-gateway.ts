...
export class SATPGateway implements IPluginWebService, ICactusPlugin {
  ...
  private static processGatewayGrpcPort(): number {
    const port = Number(process.env.SATP_GATEWAY_GRPC_PORT);
    if (process.env.SATP_GATEWAY_GRPC_PORT && !isNaN(Number(port))) {
      return parseInt(process.env.SATP_GATEWAY_GRPC_PORT);
    }
    return DEFAULT_PORT_GATEWAY_GRPC;
  }

  private static processGatewayAddress(): Address {
    return (
      (process.env.SATP_GATEWAY_ADDRESS as Address) || "http://localhost:3010"
    );
  }
  ...
  private static processGatewayIdentity(
    ...
    if (!pluginOptions.gid) {
      return {
        ...
        gatewayGrpcPort: this.processGatewayGrpcPort(),
      ...
      return {
        ...
        gatewayGrpcPort:
          gid.gatewayGrpcPort && gid.gatewayGrpcPort !== 0
            ? gid.gatewayGrpcPort
            : this.processGatewayGrpcPort(),
      ...
      try {
        ...
        const validGateway = (gateway: unknown): gateway is GatewayIdentity => {
          ...
          if (
            ...
            ("gatewayGrpcPort" in gw &&
              (typeof gw.gatewayGrpcPort !== "string" ||
                isNaN(Number(gw.gatewayGrpcPort)))) ||
