...
export class FabricTestEnvironment {
  ...
  public static readonly BRIDGE_ID =
  ...
  // Creates a JSON object containing the necessary Fabric configuration
  // to be used within the Dockerized SATP Gateway.
  ...
  // Deploys and sets up the contracts to be use in the test
  // Also creates bungeeOptions and ethereumConfig
  public async deployAndSetupContracts(claimFormat: ClaimFormat) {
    ...
    const setBridgeResponse2 = await this.apiClient.runTransactionV1({
      contractName: satpWrapperContractName,
      channelName: this.fabricChannelName,
      params: ["Org2MSP", FabricTestEnvironment.BRIDGE_ID],
      methodName: "setBridge",
      invocationType: FabricContractInvocationType.Send,
      signingCredential: this.fabricSigningCredential,
    });
