...
import { ProcessMiningAlgorithm } from "@hyperledger/cactus-plugin-ccmodel-hephaestus";
...
describe("Pausing all SATP bridges at once", () => {
  it("should create the gateway", async () => {
    //setup satp gateway
  ...
  it("should create a cross chain model", async () => {
    ...
    const model = await dispatcher!.createModel(miningAlgorithm);
    expect(model).toBeTruthy();
    expect(dispatcher!.getCCModel(miningAlgorithm)).toBeTruthy;
    dispatcher!.stopModeling();
  });
  ...
  it("should pause the bridges automatically", async () => {
    ...
    console.log(`transact response: ${res}`);
    if (res === null) {
    ...
    expect(responseBalanceOwner.success).toBeTruthy();
    expect(responseBalanceOwner.callOutput).toBe("20");
    log.info("Amount was transfer not correctly from the Owner account");
    ...
    expect(responseBalanceBridge.success).toBeTruthy();
    expect(responseBalanceBridge.callOutput).toBe("0");
    log.info("Amount was correctly not transfer to the Wrapper account");
    ...
    expect(responseBalance2.data).not.toBeUndefined();
    expect(responseBalance2.data.functionOutput).toBe("3");
    log.info("Amount was correctly not transfer to the Owner account");
