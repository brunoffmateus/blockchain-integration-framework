...
export interface NonConformingTx {
  caseID: string;
  nonConformingEventEvent?: CrossChainEvent;
  timestamp: Date;
}
...
export class CcModelHephaestus implements ICactusPlugin, IPluginWebService {
        ...
        next: async (data: IRunTransactionV1Exchange) => {
          ...
          this.log.info(
            `EVAL-${fnTag}\n` +
              ...
              `Latency ${finalTime.getTime() - data.timestamp.getTime()} ms`,
          );
        },
        error: (error: unknown) => {
          this.log.error(
  ...
  public async createModel(
    ...
    const finalTime = new Date();
    this.log.debug(
      `EVAL-${this.className}-CREATE-MODEL:` +
        `Latency: ${finalTime.getTime() - startTime.getTime()} ms`,
    );
    return ccModelFile;
  }
  ...
  private filterLogsByConformance(
    ...
    if (
      ...
      this.NonConformingTxSubject.next({
        caseID: this.caseID,
        nonConformingEventEvent: ccEvent,
        timestamp: new Date(),
      });
