...
describe("Create CCModel with Hephaestus", () => {
  ...
  it("should model the first transaction with Hephaestus", async () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      await dispatcherBesu?.Transact(req);
    ...
    expect(responseBalanceOwner.callOutput).toBe("50");
    ...
    expect(responseBalance2.data.functionOutput).toBe("50");
    log.info("Amount was transfer correctly to the Owner account");
  });
