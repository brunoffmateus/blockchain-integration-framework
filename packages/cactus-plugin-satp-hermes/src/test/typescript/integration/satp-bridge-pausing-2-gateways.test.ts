...
describe("Pausing all SATP bridges at once", () => {
  ...
  it("should pause the bridges due to besu non-conformance", async () => {
    ...
    console.log("unwrapAsset:");
    await besuBridge!.unwrapAsset(BESU_UNMODELED_ASSET_ID);
  ...
  it("should pause the bridges due to fabric non-conformance", async () => {
    hephaestus.newCaseId("NC_tx2");
    ...
    console.log("unwrapAsset:");
    await fabricBridge!.unwrapAsset(FABRIC_UNMODELED_ASSET_ID);

    hephaestus.newCaseId("try_tx2");
    // 4th transaction:
    const req = getTransactRequest(
      "mockContext",
      besuEnv,
