...
describe("Ethereum contract deploy and invoke while monitoring 3 cases", () => {
  ...
  test("monitor Ethereum transactions across 3 cases", async () => {
    ...
    await hephaestus.persistCrossChainLogCsv(
      "example-dummy-ethereum-12-events",
    );
  });

  test("Check confomity of new Ethereum transactions", async () => {
    // check conformity
    const createResEth = await apiClient.invokeContractV1({
      contract: {
        contractName: LockAssetContractJson.contractName,
    ...
    // unlock is not modeled, so this should not conform to the model
    // const unlockResEth = await apiClient.invokeContractV1({
    //   contract: {
    //     contractName: LockAssetContractJson.contractName,
    //     keychainId: keychainPlugin.getKeychainId(),
    //   },
    ...
    // expect(unlockResEth.data).toBeTruthy();
    // expect(unlockResEth.status).toBe(200);
    ...
    expect(hephaestus.numberEventsConformanceLog).toBeTruthy();
    expect(hephaestus.numberEventsConformanceLog).toEqual(3);
    expect(hephaestus.numberUnprocessedReceipts).toEqual(0);

    await hephaestus.persistCrossChainLogCsv(
      "example-dummy-ethereum-12-events-next",
      true,
    );

    const output = await hephaestus.checkConformance(
      "example-dummy-ethereum-12-events.csv",
      "example-dummy-ethereum-12-events-next.csv",
    );
    expect(output).toBeTruthy();
    log.debug(output);

    const parsedOutput = hephaestus.parseMP4PYOutput(output);
    expect(parsedOutput).toBeTruthy();
    log.debug(parsedOutput);

    // if it conforms, add to ccmodel
    // else delete from log? or keep it in a different log?
